...
function validate(p::Git, t::PackagePlan)
...
function prehook(p::Git, t::PackagePlan, pkg_dir::AbstractString)
...
function hook(p::Git, t::PackagePlan, pkg_dir::AbstractString)
    ignore = mapreduce(gitignore, vcat, t.template.plugins)
...
# Commit the files.
function posthook(p::Git, ::PackagePlan, pkg_dir::AbstractString)
    ...
    if p.manifest && !isfile(manifest)
        touch(manifest)
        cd(pkg_dir) do
            PkgCmd.withproject("Pkg.update()", false, "update manifest")
        end
    end

    LibGit2.with(GitRepo(pkg_dir)) do repo
        LibGit2.add!(repo, ".")
        msg = "Files generated by IonCLI"
