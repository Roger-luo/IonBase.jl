"""
    Readme(;
        file=".ion/templates/package/README.md",
...
function view(p::Readme, t::PackagePlan, pkg::AbstractString)
    ...
    foreach(setdiff(map(typeof, t.template.plugins), done)) do T
        append!(strings, badges(getplugin(t, T), t, pkg))
    end
    ...
    return Dict(
        ...
        "HAS_COMONICON" => hasplugin(t, Comonicon),
        "PKG" => pkg,
    )
end
