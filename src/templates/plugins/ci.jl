...
const DEFAULT_CI_VERSIONS = "[\"1.0\", <current active version>, \"nightly\"]"
const DEFAULT_CI_VERSIONS_NO_NIGHTLY = "[\"1.0\", <current active version>]"

function default_ci_versions(ion::Options.Ion=Options.read())
    return push!(default_ci_version_no_nightly(ion), "nightly")
end

function default_ci_version_no_nightly(ion::Options.Ion=Options.read())
    active_version = Options.active_julia_version(ion)
    return map(format_version, [v"1.0", active_version])
end

"""
    GitHubActions(;
        file=".ion/templates/package/github/workflows/CI.yml",
        destination="CI.yml",
        linux=true,
        osx=false,
        windows=false,
...
function view(p::GitHubActions, t::PackagePlan, pkg::AbstractString)
    os = String[]
    ...
    return Dict(
        ...
        "HAS_DOCUMENTER" => doc !== nothing && doc.depoly == "GitHubActions",
...
"""
    TravisCI(;
        file=".ion/templates/package/travis.yml",
        linux=true,
        osx=false,
        windows=false,
        x64=true,
...
function view(p::TravisCI, t::PackagePlan, pkg::AbstractString)
...
"""
    AppVeyor(;
        file=".ion/templates/package/appveyor.yml",
        x86=false,
        coverage=true,
        extra_versions=$DEFAULT_CI_VERSIONS,
    )
...
function view(p::AppVeyor, t::PackagePlan, pkg::AbstractString)
    platforms = ["x64"]
...
"""
    CirrusCI(;
        file=".ion/templates/package/cirrus.yml",
...
function view(p::CirrusCI, t::PackagePlan, pkg::AbstractString)
    return Dict(
        "HAS_CODECOV" => hasplugin(t, Codecov),
        "HAS_COVERALLS" => hasplugin(t, Coveralls),
        "HAS_COVERAGE" => p.coverage && hasplugin(t, is_coverage),
...
"""
    GitLabCI(;
        file=".ion/templates/package/gitlab-ci.yml",
        coverage=true,
...
function view(p::GitLabCI, t::PackagePlan, pkg::AbstractString)
    return Dict(
...
"""
    DroneCI(;
        file=".ion/templates/package/drone.star",
        amd64=true,
        arm=false,
        arm64=false,
        extra_versions=$DEFAULT_CI_VERSIONS_NO_NIGHTLY,
...
function view(p::DroneCI, t::PackagePlan, pkg::AbstractString)
    arches = String[]
...
"""
    collect_versions(t::PackagePlan, versions::Vector) -> Vector{String}
...
"""
function collect_versions(t::PackagePlan, versions::Vector)
...
for T in [:GitHubActions, :TravisCI, :AppVeyor, :CirrusCI]
    @eval function prehook(p::$T, t::PackagePlan, ::AbstractString)
        if isempty(p.extra_versions)
            append!(p.extra_versions, default_ci_versions(t.ion))
        end
    end
end

for T in [:GitLabCI, :DroneCI]
    @eval function prehook(p::$T, t::PackagePlan, ::AbstractString)
        if isempty(p.extra_versions)
            append!(p.extra_versions, default_ci_versions_no_nightly(t.ion))
        end
    end
end
