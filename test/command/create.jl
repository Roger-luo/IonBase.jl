...
@testset "read_template template.toml" begin
    ...
    @test GitHubActions(;file=templates("package/github/workflows/ci.yml"), extra_versions=["1.0", "1.5", "nightly"]) in plugins
    @test GitLabCI(;file=templates("package/gitlab-ci.yml"), extra_versions=["1.0", "1.5"]) in plugins
    @test TravisCI(;file=templates("package/travis.yml"), extra_versions=["1.0", "1.5", "nightly"]) in plugins
